...
export async function analyzeResume(buffer, mimetype) {
  ...
  if (!rawText || rawText.trim().length < 10) {
    // If we can't extract text at all, use mock data in demo mode
    if (isDemoMode()) {
      console.log('[DEBUG] Could not extract text, using mock data');
      return MOCK_RESUME_SKILLS;
    }
    throw new Error('Could not extract meaningful text from the resume. Ensure the file is not scanned/image-only.');
  }

  if (isDemoMode()) {
    // Even in demo mode, extract and analyze the actual resume
    const extracted = extractSkillsFromText(rawText);
    console.log('[DEBUG] Resume text length:', rawText.length);
    console.log('[DEBUG] Extracted skills:', extracted);
    // If very few skills found, supplement with mock data
    const totalFound = Object.values(extracted).reduce((sum, arr) => sum + arr.length, 0);
    if (totalFound < 3) {
      console.log('[DEBUG] Too few skills found, using mock data');
      return MOCK_RESUME_SKILLS;
    }
    return extracted;
  }
